...
#define TOGGLE_WINTER 9    // Toggle for Wintermode (D8)
#define TOGGLE_SUMMER 8    // Toggle for Summermode (D9)
...
OperationMode lastMode = MEASURE_ONLY;
...
void loop() {
  ...
  // Mode change detection for immediate display update
  if (currentMode != lastMode) {
    // Force display update immediately when mode changes
    lastDisplayUpdate = 0;
  }
...
void handleEncoder() {
  if (encoderChanged) {
    ...
    if (inEditMode) {
      ...
      // In parameter menus, encoder exits back to main menu
      else if (menuState == MENU_SET_TARGET_TEMP || 
               menuState == MENU_SET_TARGET_HUM || 
               menuState == MENU_SET_MIN_TEMP ||
               menuState == MENU_CALIBRATE) {
        if (encoderPos != 0) {
          menuState = MENU_ROOT;
          menuIndex = 0;
        }
        ...
        switch (menuState) {
          case MENU_ROOT:
            switch (menuIndex) {
              ...
              case 4: menuState = MENU_RESET_EEPROM; break;
              case 5: menuState = MAIN_DISPLAY; break; // Back to main display
            }
            break;
...
OperationMode readMode() {
  // Read the actual pin states
  bool pin8_state = digitalRead(TOGGLE_WINTER);   // D8
  bool pin9_state = digitalRead(TOGGLE_SUMMER);   // D9
  
  // With correct wiring (middle pole to GND):
  // Middle position: Both HIGH (not connected, pulled up)
  // Winter position: Pin 8 LOW (connected to GND), Pin 9 HIGH
  // Summer position: Pin 9 LOW (connected to GND), Pin 8 HIGH
  
  if (pin9_state == LOW && pin8_state == HIGH) {
    return SUMMER_MODE;   // Pin 9 connected to GND = Summer
  } 
  else if (pin8_state == LOW && pin9_state == HIGH) {
    return WINTER_MODE;   // Pin 8 connected to GND = Winter
  } 
  else {
    return MEASURE_ONLY;  // Both HIGH = Middle position = Measure Only
  }
}
...
void encoderInterrupt() {
  ...
  // Debounce: Ignore interrupts that occur too quickly (less than 5ms apart)
  ...
  // Only process on falling edge of pin A for more reliable detection
  if (lastEncoderA == HIGH && currentA == LOW) {
    if (currentB == HIGH) {
      encoderPos++;
    } else {
      encoderPos--;
