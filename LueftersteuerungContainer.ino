...
void loop() {
  ...
  // Prüfe, ob sich der Modus geändert hat und wende ggf. Offsets an
  ...
  if (!inMenu) {
    if (fanMode == AUTO) {
      ...
      static FanMode lastFanMode = fanMode;
      if (lastFanMode != fanMode) {
        displayManualMode(); // Manuellen Modus anzeigen
        lastFanMode = fanMode;
      }
    }
  } else {
...
void controlFan() {
  ...
  if (mode == SUMMER) {
    bool shouldTurnFanOn = (outsideTemp < insideTemp && outsideHum < insideHum);
  ...
  if (mode == WINTER) {
    bool shouldTurnFanOn = ((outsideTemp > minTemp && insideHum > targetHum && outsideHum < insideHum) || (outsideTemp > insideTemp));
...
// Funktion zur Anzeige des manuellen Modus auf dem LCD
void displayManualMode() {
  lcd.clear();
  lcd.setCursor(0, 0);
  // Zeigt den aktiven Modus auch im manuellen Modus an
  lcd.print(mode == SUMMER ? "S " : "W ");
  lcd.print("MANUELL");
  ...
  if (fanMode == MANUAL_ON) {
    ...
    digitalWrite(RELAY_PIN, LUEFTERHIGH); // Lüfter anschalten
  } else if (fanMode == MANUAL_OFF) {
    lcd.print("LUEFTER AUS");
    digitalWrite(RELAY_PIN, LUEFTERLOW); // Lüfter ausschalten
  }
}
...
void displayMenu() {
  ...
  switch (currentMenu) {
    ...
    case FAN_MANUAL:
      loadAndPrintProgmemStr(str_luefter);
      lcd.setCursor(0, 1);
      displayFanMenu();
      break;
...
// Funktion zur Anzeige der Menüoptionen im Hauptmenü
...
// Funktion für Lüftersteuerung
void displayFanMenu() {
  switch (menuIndex) {
    case 0: lcd.print(">Auto"); break;
    case 1: lcd.print(">Manuell An"); break;
    case 2: lcd.print(">Manuell Aus"); break;
    case 3: lcd.print(">Zurueck"); break;
  }
}

// Funktion für Kalibrierungsmenü
...
// Funktion für Innensensor-Kalibrierung
void displayCalibrateInsideMenu() {
  switch (menuIndex) {
...
// Funktion für Außensensor-Kalibrierung
void displayCalibrateOutsideMenu() {
  switch (menuIndex) {
    case 0: lcd.print(">Temperatur"); break;
    case 1: lcd.print(">Luftfeuchtigkeit"); break;
    case 2: lcd.print(">Zurueck"); break;
  ...
  switch (currentMenu) {
    ...
    case FAN_MANUAL:
      if (menuIndex == 0) {
        ...
        fanMode = MANUAL_ON;
        digitalWrite(RELAY_PIN, LUEFTERHIGH); // Direkt aktivieren
        setCurrentMenu(MAIN); // Zurück zum Hauptmenü
      }
      else if (menuIndex == 2) {
        fanMode = MANUAL_OFF;
        digitalWrite(RELAY_PIN, LUEFTERLOW); // Direkt deaktivieren
        setCurrentMenu(MAIN); // Zurück zum Hauptmenü
      }
      else if (menuIndex == 3) {
        setCurrentMenu(MAIN); // Zurück zum Hauptmenü
...
// Methode zur Aktualisierung des Menüindex
void updateMenuIndex(int direction) {
  menuIndex += direction;
  limitMenuIndex();
}
...
void loadSettings() {
  ...
  if (isnan(targetTemp)) targetTemp = 25.0;
  if (isnan(targetHum)) targetHum = 60.0;
